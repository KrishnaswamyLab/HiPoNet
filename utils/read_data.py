...
def load_data(raw_dir, full, population_cache=None, pdo_caf_cache=None):
    ...
    if data_name in {"population_npz", "pdo_caf_mendeley", "pdo_caf"}:
        cache_path = pdo_caf_cache
        if cache_path is None and data_name != "population_npz":
            root = (
                os.path.join("data", "pdo_caf_mendeley")
                if raw_dir in {"pdo_caf_mendeley", "pdo_caf"}
                else raw_dir
            )
        ...
        return load_population_cache(cache_path)
